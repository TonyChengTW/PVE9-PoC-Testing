#!/bin/bash
# 20Gbps LACP bond0 压力测试脚本
# 源主机: 172.19.0.171 (bond0: 2x10G LACP)
# 目标主机: 172.19.0.173 (bond0: 2x10G LACP)
# 目标: 验证 20Gbps 聚合带宽
...
echo "=========================================="
echo "20Gbps LACP bond0 压力测试开始"
echo "源主机: $(hostname) ($(ip addr show bond0 2>/dev/null | grep 'inet ' | head -1 | awk '{print $2}'))"
echo "目标主机: $TARGET"
echo "bond0 模式: $(cat /proc/net/bonding/bond0 2>/dev/null | grep 'Bonding Mode' | cut -d: -f2)"
echo "传输哈希: $(cat /proc/net/bonding/bond0 2>/dev/null | grep 'Transmit Hash' | cut -d: -f2)"
...
run_test() {
    ...
    if [ $? -eq 0 ]; then
        # 提取带宽
        local bw=$(cat "$output_file" | python3 -c "import sys,json; d=json.load(sys.stdin); print('{:.2f}'.format(d.get('end',{}).get('sum_received',{}).get('bits_per_second', d.get('end',{}).get('sum',{}).get('bits_per_second',0))/1e9))" 2>/dev/null)
        echo "  -> 完成: ${bw:-N/A} Gbps"
    else
        echo "  -> 失败"
    fi
